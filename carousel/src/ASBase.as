...
package 
{
	...
	public class ASBase extends ObjectContainer3D
	{
		...
		{
			var bmd:BitmapData = new BitmapData(2,2, false, 0xCCCCCC);
		...
		{
			AssetLoader(e.currentTarget).removeEventListener(AssetEvent.ASSET_COMPLETE, onBitmapDataComplete);
		...
		protected function loadLightMapMethod(material:MaterialBase, blendMode:String, useSecondaryUV:Boolean, url:String):LightMapMethod
		{
			var bmd:BitmapData = new BitmapData(2,2, false, 0xFFFFFF);
			var bitmapTexture:BitmapTexture = new BitmapTexture(bmd);
			bitmapTexture.name = url;
			var lightMap:LightMapMethod = new LightMapMethod(bitmapTexture, blendMode, useSecondaryUV);
			SinglePassMaterialBase(material).addMethod(lightMap);
			loadDependency(url, onTextureDataComplete);

			return lightMap;
		}

		private function onTextureDataComplete(e:AssetEvent):void
		{
			AssetLoader(e.currentTarget).removeEventListener(AssetEvent.ASSET_COMPLETE, onTextureDataComplete);
			finalizeTextureBitmapData(e.assetPrevName, BitmapTexture(e.asset).bitmapData);
		}

		private function finalizeTextureBitmapData(id:String, bitmapData:BitmapData):void
		{
			var spmb:SinglePassMaterialBase;
			var j:uint;
			var method:EffectMethodBase;
			for(var i:uint = 0;i<_materials.length;++i){
				spmb = SinglePassMaterialBase(_materials[i]);
				if(spmb.numMethods != 0){
					for(j = 0;j<spmb.numMethods;++j){
						method = spmb.getMethodAt(j);
						if(method is LightMapMethod && LightMapMethod(method).texture.name == id){
							BitmapTexture(LightMapMethod(method).texture).bitmapData.dispose();
							BitmapTexture(LightMapMethod(method).texture).bitmapData = bitmapData;
							return;
						}
					}
				}
